...
if mode == "test":
  ...
  result_folder = "F:\\WJ\\SincNet\\data_split\\Libri\\result_noise_set"
  result_path = glob.glob(result_folder + '/*')

  f = open('result_noise.txt',"w")

  with torch.no_grad():
      # path = "F:\\WJ\\SincNet\\data_split\\Libri\\dataset\\spk155_105.flac"
      # path = "F:\\WJ\\SincNet\\data_split\\Libri\\result_nosie_set\\test2658.flac"
      ...
      # result_set
        ...
        signal = torch.from_numpy(signal).float().cuda().contiguous()
        # lab_batch = lab_dict["spk155_105.flac"]
        ...
        [val, best_class] = torch.max(torch.sum(pout, dim=0), 0)
        # err_sum_snt = err_sum_snt + (best_class != lab[0]).float()
        f.write(str(int(best_class))+"\n")

        print(str(int(best_class)))
